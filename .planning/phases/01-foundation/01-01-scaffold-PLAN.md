...
must_haves:
  truths:
    ...
    - "The repo has a working `.gitignore` excluding node_modules, dist, and .env"
    - "The Vite app is isolated under `app/` so the live root `index.html` (Docsify entry) is untouched"
  artifacts:
    - path: ".gitignore"
      provides: "Excludes node_modules, dist, .env, .DS_Store, transient script outputs"
...
Do NOT add `.env.example` to the gitignore — that file is meant to be tracked. Do NOT add `package-lock.json` — lockfiles must be committed.
  ...
  <verify>
    <automated>test -f .gitignore && grep -q "^node_modules/$" .gitignore && grep -q "^dist/$" .gitignore && grep -q "^\.env$" .gitignore && grep -q "^\.DS_Store$" .gitignore && grep -q "^README.proposed.md$" .gitignore</automated>
  </verify>
  <acceptance_criteria>
  ...
  <done>The repo has a .gitignore covering node_modules, dist, .env, .DS_Store, and the three transient outputs of scripts/update-spec.mjs.</done>
</task>
